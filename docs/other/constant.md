...
如一个记录答案的 res 变量，现代编译器（如洛谷默认开启的 GCC O2 优化）非常聪明，它确切地知道 res 只在这个函数内使用，没有外部作用域会干扰它。因此，编译器会直接把 res 放进 CPU 的超高速寄存器（Register）中。上亿次的 `res++` 和 `res--` 都在 CPU 内部瞬间完成，根本不需要访问内存。

## lambda 表达式

开启优化时，与全局函数相比，编译器更容易把 lambda 表达式自动内联。