...
::: details 解析
共享内存通信速度快，但通信进程必须借助锁、信号量等机制自行保证同步；消息传递由 `send`、`receive` 和内核消息队列提供较明确的同步边界，但通常涉及系统调用和数据复制。
:::
...
::: info 例 8
为什么记录型信号量值为负时，其绝对值通常可以表示等待进程数？
:::

::: details 解析
每次 `wait` 都先将信号量减 $1$，资源不足时进程进入等待队列；每增加一个等待进程，信号量值就再减 $1$。因此在这种定义下，负值的绝对值对应等待队列中的进程数量。
:::
