...
具体而言，将区间 $[l_1,r_1],[l_2,r_2]$ 拆分成 $[l_1,l_1+2^k-1],[l_2,l_2+2^k-1]$ 和 $[r_1-2^k+1,r_1],[r_2-2^k+1,r_2]$，其中 $k=\log(r-l+1)$。

维护 $k$ 层并查集，每一层中并查集节点表示 $x$ 开始长度为 $2^k$ 的等价区间集合。

从 $\log n$ 层向下传递信息，维护到第 $0$ 层即可。

时间复杂度：$O(n\log n\alpha(n))$，仅保留相邻两层，空间复杂度可优化至 $O(n)$。
