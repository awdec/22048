...
struct cat_segment
{
    ...
    int *row(int k)
    {
        return st.data() + 1LL * k * m;
    }
    const int *row(int k) const
    {
        return st.data() + 1LL * k * m;
    }
...
​但是猫树比 ST 表功能更加强大，猫树能够维护不满足幂等性的操作，ST 表只能维护幂等性操作。ST 表查询时，会将区间 $[l,r]$ 转换成可能相交的区间，可以维护最值这种区间交集重复考虑不影响结果的信息。反之，ST 表要将区间拆分成 $O(\log n)$ 段，那就不是 $O(1)$ 的查询了。
